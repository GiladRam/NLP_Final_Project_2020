...
RANDOM_UNIFORM_SEEDER = 123
FLAGS = flags.FLAGS


class ExtractorModel(object):
    ...
    def _add_placeholders(self):
        ...
        hps = self._hps
        self._art_batch = tf.placeholder(tf.int32, [hps.batch_size, hps.max_art_len, hps.max_sent_len],
                                         name='art_batch')
    ...
    def _make_feed_dict(self, batch, just_enc=False):
        ...
        """
        feed_dict = {
            self._art_batch: batch.art_batch,
            self._art_lens: batch.art_lens,
            self._sent_lens: batch.sent_lens,
            self._art_padding_mask: batch.art_padding_mask,
            self._sent_padding_mask: batch.sent_padding_mask
        }
        if self._graph_mode == 'compute_loss':
    ...
    def _add_encoder(self, encoder_inputs, seq_len, name):
        ...
        with tf.variable_scope(name):
            ...
            (encoder_outputs, (_, _)) = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, encoder_inputs,
    ...
    def _add_classifier(self, sent_feats, art_feats):
        ...
        with tf.variable_scope('classifier'):
            w_content = tf.get_variable('w_content', [hidden_dim, 1], dtype=tf.float32,
                                        initializer=self.truncated_normal_initializer)
            w_salience = tf.get_variable('w_salience', [hidden_dim, hidden_dim], dtype=tf.float32,
                                         initializer=self.truncated_normal_initializer)
            w_novelty = tf.get_variable('w_novelty', [hidden_dim, hidden_dim], dtype=tf.float32,
                                        initializer=self.truncated_normal_initializer)
            ...
            for i in range(self._hps.max_art_len):
                ...
                salience_feats = tf.reduce_sum(tf.matmul(sent_feats[:, i, :], w_salience) * art_feats, 1,
                                               keep_dims=True)  # (batch_size, 1)
                novelty_feats = tf.reduce_sum(tf.matmul(sent_feats[:, i, :], w_novelty) * tf.tanh(s), 1,
                                              keep_dims=True)  # (batch_size, 1)
    ...
    def _add_sentence_selector(self):
        """Add the whole sequence-to-sequence model to the graph."""
        hps = self._hps
        vocabulary_size = self._vocab.size()

        with tf.variable_scope('SentenceSelector'):
            # Some initializers
            self.random_uniform_initializer = tf.random_uniform_initializer(minval=-self._hps.rand_unif_init_mag,
                                                                            maxval=self._hps.rand_unif_init_mag,
                                                                            seed=RANDOM_UNIFORM_SEEDER)
            self.truncated_normal_initializer = tf.truncated_normal_initializer(stddev=hps.trunc_norm_init_std)

            # Add embedding matrix (shared by the encoder and decoder inputs)
            with tf.variable_scope('embedding'):
                self.embedding = tf.get_variable('embedding',
                                                 [vocabulary_size, hps.emb_dim],
                                                 dtype=tf.float32,
                                                 initializer=self.truncated_normal_initializer)
                if hps.mode == "train":
                    self._add_emb_vis(self.embedding)  # add to tensorboard
                emb_batch = tf.nn.embedding_lookup(self.embedding,
                                                   self._art_batch)  # tensor with shape (batch_size, max_art_len, max_sent_len, emb_size)

            # Add the two encoders
            # Add Word-Level encoder to encode each sentence.
            sentence_encoder_inputs = tf.reshape(emb_batch, [-1, hps.max_sent_len, hps.emb_dim])  # (batch_size*max_art_len, max_sent_len, emb_dim)
            sentence_lengths = tf.reshape(self._sent_lens, [-1])  # (batch_size*max_art_len, )
            sentence_encoder_outputs = self._add_encoder(sentence_encoder_inputs,
                                                         sentence_lengths,
                                                         name='sentence_encoder')  # (batch_size*max_art_len, max_sent_len, hidden_dim*2)
            ...
            sentence_padding_mask = tf.reshape(self._sent_padding_mask, [-1, hps.max_sent_len, 1])  # (batch_size*max_art_len, max_sent_len, 1)
            sent_lens_float = tf.reduce_sum(sentence_padding_mask, axis=1)
            self.sent_lens_float = tf.where(sent_lens_float > 0.0,
                                            sent_lens_float,
                                            tf.ones(sent_lens_float.get_shape().as_list()))
            art_enc_inputs = tf.reduce_sum(sentence_encoder_outputs * sentence_padding_mask,
                                           axis=1) / self.sent_lens_float  # (batch_size*max_art_len, hidden_dim*2)
            art_enc_inputs = tf.reshape(art_enc_inputs, [hps.batch_size, -1,
                                                         hps.hidden_dim_selector * 2])  # (batch_size, max_art_len, hidden_dim*2)
            art_enc_outputs = self._add_encoder(art_enc_inputs, self._art_lens,
                                                name='art_encoder')  # (batch_size, max_art_len, hidden_dim*2)

            # Get each sentence representation and the document representation.
            sent_feats = tf.contrib.layers.fully_connected(art_enc_outputs, hps.hidden_dim_selector,
                                                           activation_fn=tf.tanh)  # (batch_size, max_art_len, hidden_dim)
            art_padding_mask = tf.expand_dims(self._art_padding_mask, 2)  # (batch_size, max_art_len, 1)
            art_feats = tf.reduce_sum(art_enc_outputs * art_padding_mask, axis=1) / tf.reduce_sum(art_padding_mask,
                                                                                                  axis=1)  # (batch_size, hidden_dim)
            art_feats = tf.contrib.layers.fully_connected(art_feats, hps.hidden_dim_selector,
                                                          activation_fn=tf.tanh)  # (batch_size, hidden_dim)
