...
def run_training(model, batcher, sess_context_manager, sv, summary_writer, pretrained_saver=None, saver=None):
    ...
    with sess_context_manager as sess:
        ...
        for _ in range(FLAGS.max_train_iter):  # repeats until interrupted
            ...
            if not batch:
                tf.logging.info('training has finished - no more batches left...')
                return
...
def run_eval(model, batcher):
    ...
    while True:
        ...
        running_avg_ratio = util.calc_running_avg_loss(ratio, running_avg_ratio, summary_writer, train_step,
                                                       'running_avg_ratio')

        # If running_avg_loss is best so far, save this checkpoint (early stopping).
        # These checkpoints will appear as bestmodel-<iteration_number> in the eval dir
        if best_ratio is None or running_avg_ratio < best_ratio:
            tf.logging.info('Found new best model with %.3f running_avg_ratio. Saving to %s', running_avg_ratio,
                            bestmodel_save_path)
