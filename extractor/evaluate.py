...
class SelectorEvaluator(object):
        ...
        while True:
            ...
            if batch is None:  # finished decoding dataset in single_pass mode
                ...
                if FLAGS.eval_gt_rouge:
                    tf.logging.info("Output has been saved in %s and %s. Starting ROUGE eval...", self._rouge_ref_dir,
                                    self._rouge_gt_dir)
                    ...
                    results_log(self._precision, self._recall, self._accuracy, self._select_sent_num, self._ratio,
                                self._decode_dir)
            ...
            if FLAGS.eval_gt_rouge:
                self.write_for_rouge(original_abstract_sents, original_select_sents,
            ...
            if FLAGS.single_pass:
                self.write_for_rouge(original_abstract_sents, select_sents,
                                     counter)  # write ref summary and decoded summary to file, to eval with pyrouge later
                if FLAGS.save_pkl:
    ...
    def save_result(self, article_sents, reference_sents, select_sents, select_ids, gt_ids, precision, recall, acc,
                    index):
...
def get_decode_dir_name(ckpt_name, dataset):
    ...
    if FLAGS.select_method == 'num':
        ...
        dirname = "%s_%.2fthres_%iminselect_%imaxselect" % (
        FLAGS.select_method, FLAGS.thres, FLAGS.min_select_sent, FLAGS.max_select_sent)
