...
class UnifiedModel(object):
    ...
    def __init__(self, hps, extractor_model, abstract_model):
        self._hps = hps
    ...
    def _add_inconsistent_loss(self):
        hps = self._hps
        enc_len = tf.shape(self._abstract_model._enc_sent_id_mask)[1]

        batch_nums = tf.expand_dims(tf.range(0, limit=hps.batch_size), 1)  # shape (batch_size, 1)
        indices = tf.stack((tf.tile(batch_nums, [1, enc_len]), self._abstract_model._enc_sent_id_mask),
        ...
        selector_probs_projected = tf.gather_nd(self._extractor_model.probs, indices)  # shape (batch_size, enc_len)
        ...
        for dec_step, attn_dist in enumerate(self._abstract_model.attn_dists_norescale):
            ...
            loss_one_step *= self._abstract_model._dec_padding_mask[:, dec_step]  # shape (batch_size,)
            losses.append(loss_one_step)
        loss = tf.reduce_mean(sum(losses) / tf.reduce_sum(self._abstract_model._dec_padding_mask, axis=1))
        return loss
    ...
    def _add_train_op(self):
        ...
        loss_to_minimize = self._abstract_model._total_loss if hps.coverage else self._abstract_model._loss
        loss_to_minimize += (self._extractor_model._loss * hps.selector_loss_wt)
    ...
    def build_graph(self):
        ...
        t0 = time.time()
        self._extractor_model._add_placeholders()
        self._abstract_model._add_placeholders()
        with tf.device("/gpu:0"):
            self._extractor_model._add_sentence_selector()
            self._abstract_model._add_seq2seq(selector_probs=self._extractor_model.probs)
            if self._hps.inconsistent_loss and self._abstract_model._graph_mode != 'greedy_search':
    ...
    def run_train_step(self, sess, batch):
        ...
        hps = self._hps
        feed_dict = self._extractor_model._make_feed_dict(batch)
        feed_dict.update(self._abstract_model._make_feed_dict(batch))

        to_return = {
            'train_op': self._train_op,
            'summaries': self._summaries,
            'p_gen_avg': self._abstract_model.p_gen_avg,
            'probs': self._extractor_model.probs,
            'loss': self._abstract_model._loss,
            'global_step': self.global_step,
        }
        if hps.coverage:
            to_return['coverage_loss'] = self._abstract_model._coverage_loss
        if hps.inconsistent_loss:
            to_return['inconsist_loss'] = self._inconsistent_loss

        to_return['selector_loss'] = self._extractor_model._loss
    ...
    def run_greedy_search(self, sess, batch):
        ...
        feed_dict = self._extractor_model._make_feed_dict(batch)
        feed_dict.update(self._abstract_model._make_feed_dict(batch))
        return sess.run(self._abstract_model.greedy_search_words, feed_dict)
