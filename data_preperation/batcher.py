    ...
    def __init__(self, article_sentences, extract_ids, abstract_sentences, vocab, hps):
        ...
        if hps.model in ['rewriter', 'end2end']:
            ...
            self.enc_input = [vocab.word2id(w) for w in
                              enc_input_words]  # list of word ids; OOVs are represented by the id for UNK token
...
class Batch(object):
    ...
    def __init__(self, example_list, hps, vocab):
        ...
        if hps.model in ['rewriter', 'end2end']:
            ...
            self.init_rewriter_decoder_seq(example_list,
                                           hps)  # initialize the input and targets for the rewriter decoder
        if hps.model in ['selector', 'end2end']:
...
class Batcher(object):
    ...
    def fill_example_queue(self):
        ...
        while True:
            ...
            abstract_sentences = [sent.strip() for sent in data.document2sents(
            ...
            example = Example(article_sentences, extract_ids, abstract_sentences, self._vocab,
    ...
    def fill_batch_queue(self):
        ...
        """
        while True:
            if ((self._hps.mode == 'evalall' and self._hps.decode_method == 'beam')
                    or (self._hps.mode == 'eval' and self._hps.eval_method == 'rouge'
                        and self._hps.decode_method == 'beam')):
