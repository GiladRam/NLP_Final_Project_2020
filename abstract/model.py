...
class AbstractModel(object):
    ...
    def _add_placeholders(self):
        ...
        if hps.model == 'end2end':
            self.enc_sent_id_mask = tf.placeholder(tf.int32, [hps.batch_size, None], name='enc_sent_id_mask')
        ...
        if self._graph_mode in ['teacher_forcing', 'beam_search']:
            ...
            if self._graph_mode == 'beam_search':
                self.prev_context = tf.placeholder(tf.float32, [hps.batch_size, hps.hidden_dim_rewriter * 2],
                                                   name='prev_context')
                if hps.coverage:
                    self.prev_coverage = tf.placeholder(tf.float32, [hps.batch_size, None], name='prev_coverage')

    def _make_feed_dict(self, batch, is_encoder_only=False):
        """Make a feed dictionary mapping parts of the batch to the appropriate placeholders.

        Args:
          batch: Batch object
          is_encoder_only: Boolean. If True, only feed the parts needed for the encoder.
        """
        hps = self._hps
        feed_dict = {
        ...
        if hps.model == 'end2end':
            feed_dict[self.enc_sent_id_mask] = batch.enc_sent_id_mask
    ...
    def _add_encoder(self, encoder_inputs, seq_len):
        ...
        with tf.variable_scope('encoder'):
            cell_fw = tf.contrib.rnn.LSTMCell(self._hps.hidden_dim_rewriter, initializer=self.rand_unif_init,
                                              state_is_tuple=True)
            cell_bw = tf.contrib.rnn.LSTMCell(self._hps.hidden_dim_rewriter, initializer=self.rand_unif_init,
                                              state_is_tuple=True)
    ...
    def _add_decoder_one_step(self, inputs):
        ...
        if hps.model == 'end2end':
            ...
            enc_sent_id_mask = self.enc_sent_id_mask
        else:
            selector_probs = None
            enc_sent_id_mask = None

        output, out_state, attn_dist_norescale, attn_dist, context_vector, p_gen, coverage = \
            attention_decoder_one_step(inputs, self._dec_out_state, self._enc_states, self._enc_padding_mask, cell, \
            ...
            for i in range(dec_steps):
                ...
                if i == 0:
                    ...
                    if self._graph_mode in ['teacher_forcing', 'greedy_search']:
                        ...
                        inp = tf.tile(tf.constant([self._vocab.word2id(data.START_DECODING)]),
                                      [hps.batch_size])  # shape (batch_size,)
                    ...
                    if self._graph_mode == 'teacher_forcing':
                        ...
                        unk_batch = tf.tile(tf.constant([self._vocab.word2id(data.UNKNOWN_TOKEN)]),
                                            [hps.batch_size])  # shape (batch_size,)
    ...
    def _calc_final_dist_one_step(self, vocab_dist, attn_dist, p_gen):
        with tf.variable_scope('final_distribution'):
            ...
            vocab_dist_extended = tf.concat(axis=1,
                                            values=[vocab_dist, extra_zeros])  # shape (batch_size, extended_vsize)
    ...
    def _add_seq2seq(self, selector_probs=None):
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('embedding'):
                ...
                if self._graph_mode in ['teacher_forcing', 'beam_search']:
                    dec_inputs = tf.unstack(self._dec_batch,
                                            axis=1)  # list of max_dec_steps tensor with shape (batch_size,)
            ...
            # Our encoder is bidirectional and our decoder is unidirectional so we need to reduce the final encoder hidden
            # state to the right size to be the initial decoder hidden state
            ...
            if self._graph_mode == 'teacher_forcing':
                with tf.variable_scope('loss'):
                    ...
                    for dec_step, log_dist in enumerate(log_dists):
                        ...
                        losses = tf.gather_nd(-log_dist,
                                              indices)  # shape (batch_size). loss on this step for each batch
        ...
        if self._graph_mode == 'beam_search':
            ...
            self._topk_log_probs, self._topk_ids = tf.nn.top_k(log_dists,
                                                               hps.batch_size * 2)  # note batch_size=beam_size in decode mode
    ...
    def run_encoder(self, sess, batch):
        ...
        """
        feed_dict = self._make_feed_dict(batch, is_encoder_only=True)  # feed the batch into the placeholders
    ...
    def decode_onestep(self, sess, batch, latest_tokens, enc_states, dec_init_states, \
        ...
        if self._hps.model == 'end2end':
            ...
            feed[self.enc_sent_id_mask] = batch.enc_sent_id_mask
        ...
        return results['ids'], results[
            'probs'], new_states, attn_dists_norescale, attn_dists, new_context, p_gens, new_coverage
